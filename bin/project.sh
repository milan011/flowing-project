...
start() {
  ...
  if [ $? -eq "1" ]; then
    ...
    # jar服务启动脚本
    nohup java $JAVA_OPTS -Xloggc:$LOG_FILE/gc/gclog.log -XX:HeapDumpPath=$LOG_FILE/gc/HeapDump.hprof -jar $JAR_PATH/$JAR_NAME --spring.profiles.active=$PROFILES_ACTIVE >./$LOG_FILE/run.log 2>&1 &
    echo $! > $JAR_PID
